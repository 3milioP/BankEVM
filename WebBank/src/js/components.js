const Main = () => {
  ...
  const [receiverAddress, setReceiverAddress] = React.useState(""); // Dirección del receptor
  ...
  React.useEffect(() => {
    const handleAccountsChanged = (accounts) => {
      if (accounts.length === 0) {
        ...
        setAccount(accounts[0]); // Actualiza la cuenta conectada
      }
    };
    ...
    const connectToMetamask = async () => {
      try {
        ...
        if (savedWallet) {
          ...
          // Si la wallet está guardada, inicializamos web3 y BANK
        ...
        // Inicializar Web3 y el contrato de BANK al conectar la wallet
        const web3 = new Web3(window.ethereum);
        setWeb3Instance(web3);
        const bank = new web3.eth.Contract(BANK_ABI, BANK_ADDRESS);
        BANK = bank; // Asegúrate de que BANK esté definido
  ...
  const transfer = async () => {
    ...
    try {
      // Validar que receiverAddress tenga valor
      if (!receiverAddress || !web3Instance.utils.isAddress(receiverAddress)) {
  ...
  return (
    <div>
      ...
      <p>Interest Rate: {interestRate}%/day</p>
